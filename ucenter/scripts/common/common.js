...
if (getUrl('token')) {
    localStorage.token = getUrl('token');
}

var getToken = function() {
    return getCookie('token') || getUrl('token') || localStorage.token;
};
...
function getElementsByClassName(idWrap, clsName, tagName) {
    var elements = [],
        children = idWrap.getElementsByTagName(tagName);
...
binApp.addEvent = function(elem, type, handler, test) {
    ...
    for (var key in elem.$events[type]) {
        ...
        if (elem.$events[type][key].toString() == handler.toString()) {
            return;
        }
    }
    elem.$guid++;
...
binApp.bind = function(dom, type, fn, fnName) {
    ...
    if (binApp.checkEvent(dom, type, fn).bool) {
        return;
    }
    if (!fnName) {
    ...
    if (!fnName) {
        ...
        switch (typeof fnName) {
            case 'function':
                ...
                if (C.bool) {
                    delete dom.eventQueue[type][C.guid];
                }
                break;
            case "string":
...
binApp.domElemBit = function(elem, opts, time) {
    ...
    var iScrollTop = (binApp.pageY(elem) - ("negNum" in opts ? opts.negNum : 0)),
        t = 0,
        d = time,
...
binPP.forEvent = function(str, opts, callback, callwith, confirmCallback) {
    ...
    oP.off = function() {
        ...
        if (oP.maskFlag) binApp.animate(binPP.oMask, {
            mode: 'Quint',
            effect: 'easeIn'
        }, {
            opacity: 0
    ...
    oP.keydown = function(event) {
        ((event || window.event).keyCode == 13) ? oP.forDefineFN(): null;
    };
    oP.forDefineFN = function() {
        oP.confirmCallback = confirmCallback;
        oP.off();
    };
    oP.forCloseFN = function() {
        oP.confirmCallback = false;
        oP.off();
    };
    oP.forBtnFN = function() {
        oP.oClose = getElementsByClassName(oP, "close", "a");
        binApp.each(oP.oClose, function() {
            binApp.addEvent(this, "click", oP.forCloseFN);
        });
        oP.oDefine = getElementsByClassName(oP, "define", "a");
        binApp.each(oP.oDefine, function() {
            binApp.addEvent(this, "click", oP.forDefineFN);
        });
...
if (!window.binJDCSS) {
    ...
    binApp.insertCssRule([{
        name: ".binWinMask",
        cssText: "position:fixed;width:100%;height:100%;left:0;top:0;background:black;opacity:0;filter:alpha(opacity=0);z-index:9898;_position:absolute;_width:expression(document.documentElement.clientWidth+'px');_height:expression(document.documentElement.scrollHeight+'px');"
    }, {
        name: ".binPP_mBg",
        cssText: "width:100%;height:100%;opacity:1;filter:alpha(opacity=100);position:absolute;left:-5px;top:-5px;box-shadow:1px 1px 0 white,-1px -1px 0 white,0 2px 15px rgba(35,35,35,.5);text-shadow:#fff 1px 1px 2px;border-radius:5px;background:#fff;border: 5px solid #eaeaea;"
    }, {
        name: ".binPP_middle",
        cssText: "position:fixed;z-index:98168;left: 50%;top:0;margin:0;min-width:280px;max-width:98%;height:auto;line-height:21px;text-align: center;color:#606060;font-size:14px;font-weight: normal;_position:absolute;_width:expression(this.offsetWidth<280?280+'px':true);"
    }, {
        name: ".binPP_info",
        cssText: "position:relative;z-index:168;padding:35px 25px;line-height:21px;"
    }, {
        name: ".binPP_top",
        cssText: "position:fixed;z-index:98168;left: 50%;top:0;margin:0;min-width:280px;max-width:98%;height:auto;line-height:21px;text-align: center;color:#606060;font-size:14px;font-weight: normal;_position:absolute;_width:expression(this.offsetWidth<280?280+'px':true);"
    }, {
        name: ".binPP_top .binPP_mBg",
        cssText: "border-radius:0 0 10px 10px;"
    }, {
        name: ".binPP_top .binPP_info",
        cssText: "padding:135px 25px 15px;"
    }, {
        name: ".binPP_ft_bar",
        cssText: "padding:5px 5px 0;position:relative;z-index:10;min-width:358px;background-color: #eaeaea;border-top:1px solid #ccc;box-shadow:0 1px 0 #fff inset;"
    }, {
        name: ".binPP_ft_bar a",
        ...
        name: ".binPP_ft_bar a:hover",
        cssText: "text-decoration: none;border-color:#90d2ed;color:#007fb0;background:#dff4fc;background:-webkit-linear-gradient(90deg, #dff4fc, white);background:-moz-linear-gradient(90deg, #dff4fc, white);background:-o-linear-gradient(90deg, #dff4fc, white);filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='white', endColorstr='#dff4fc', GradientType='0');text-shadow:1px 1px 0 white;"
    }, {
        name: ".binPP_hd_bar",
        cssText: "text-indent:10px;text-align:left;height:30px;line-height:22px;position:absolute;left:0;top:0;width:100%;z-index:100;background-color: #eaeaea;border-bottom:1px solid #ccc;box-shadow:0 -1px 0 #fff inset;color:#333;cursor:move;"
    }, {
        name: ".binPP_hd_bar .close-btn",
        cssText: "text-indent:0;position:absolute;right:5px;top:2px;line-height:100%;padding:3px 5px;display:block;border-radius:3px;color:#b1b1b1;font:12px/1 'lucida Grande',Verdana,Arial;"
    }, {
        name: ".binPP_hd_bar .close-btn:hover",
        cssText: "text-decoration: none;background:#ccc;color:white;text-shadow:-1px -1px 0 #c1c1c1;"
    }, {
        name: ".binDrag",
        cssText: "cursor: move;"
    }, {
        name: ".oFullScreenLoading",
        cssText: "position:fixed;z-index:9999;width:68px;height:68px;margin:-34px 0 0 -34px;left:50%;top:50%;background:#101010 url(imgs/loading.gif) no-repeat center center;border-radius:8px;_position:absolute;_top:expression(document.documentElement.scrollHeight+document.documentElement.clientHeight/2+'px');"
    }]);
}
...
binApp.winMask = function(obj, opts) {
    ...
    if (opts.animate) {
        binApp.animate(
            obj.oMask, ('tween' in opts.animate ? opts.animate.tween : {
                mode: 'Quint',
                effect: 'easeOut'
            }), ('css' in opts.animate ? opts.animate.css : {
                opacity: 30
...
binApp.loadingIn = function(opts, callback) {
    ...
    if (opts.innerHTML) {
        binLD.oFSLD.innerHTML = opts.innerHTML;
    }
...
binApp.loadingOut = function(callback) {
    ...
    binApp.animate(binLD.oFSLD.oMask, {
        mode: 'Quint',
        effect: 'easeIn'
    }, {
        opacity: 0
